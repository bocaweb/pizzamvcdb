...
public class DAOPizzaOrder {
    ...
    public static void addOrder(PizzaOrder order) {
        final String QUERY = "insert into order_table "
                + "(id, email, size, toppings, orderTime) "
                + "VALUES (null, ?, ?, ?, now())";
        try (
    ...
    public static List<PizzaOrder> getOrderList() {
        ...
        try (
            ...
            while (rs.next()) {
                myList.add(new PizzaOrder(rs.getInt("id"), 
                        rs.getString("email"), rs.getString("size"), 
                        rs.getString("toppings"), rs.getString("orderTime")));
            }
        } catch (SQLException ex) {
            Logger.getLogger(DAOPizzaOrder.class.getName()).log(Level.SEVERE, null, ex);
        }
